# =====================================================================================
# soliton_solver/theories/spin_triplet_superconducting_magnet/initial_config.py
# =====================================================================================
"""
Initial condition / ansatz CUDA kernels for the superferro simulation.
...
def create_ground_state_kernel(Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i, p_f):
    ...
    - p_f: float params (expects p_f[20]=M0, p_f[11]=u1, p_f[12]=u2)
    """
    x, y = cuda.grid(2)
...
def create_initial_configuration_kernel(Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, ansatz_bloch, ansatz_neel, ansatz_anti, skyrmion_rotation, vortex_number, p_i, p_f):
    ...
    - p_f expects:
        p_f[0]=xsize, p_f[1]=ysize, p_f[11]=u1, p_f[11]=u2, p_f[16]=ainf, p_f[21]=skN (skyrmion number)
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
...
def create_vortex_kernel(Field, grid, pxi, pxj, vortex, p_i, p_f):
    ...
    - p_f expects p_f[11]=u1, p_f[12]=u2, and p_f[15]=vortex_number (winding)
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
    ...
    # Complex composition: (old * int) / u2
...
def create_skyrmion_kernel(Field, grid, pxi, pxj, skyrmion_rotation, ansatz_bloch, ansatz_neel, ansatz_anti, p_i, p_f):
    ...
    - p_f expects p_f[21]=skN (skyrmion number)
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
