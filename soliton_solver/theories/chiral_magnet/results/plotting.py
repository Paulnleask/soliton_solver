...
# =========================
"""
To run: python -m soliton_solver.theories.chiral_magnet.results.plotting
"""
import numpy as np
import matplotlib
...
MagneticChargeDensity = load_dat("MagneticChargeDensity.dat")
...
draw_panel(ax11, xGrid, yGrid, EnergyDensity,
           r"$\mathcal{E}(\vec{x})$", xLength, yLength, xlim=(xMin,xMax), ylim=(yMin,yMax))

# Magnetic charge density
ax12 = fig1.add_subplot(gs[0, 1])
draw_panel(ax12, xGrid, yGrid, MagneticChargeDensity,
           r"$\rho(\vec{x})=-\vec{\nabla}\cdot\vec{n}$", xLength, yLength, xlim=(xMin,xMax), ylim=(yMin,yMax),cmap="bone")

# Runge coloring (use the RGB image directly)
ax13 = fig1.add_subplot(gs[0, 2])
im = ax13.imshow(rgbOutput,
                 origin="lower",
...
inset.imshow(ps_rgb,
             origin="lower",
             extent=[ps_sigma.min(), ps_sigma.max(), ps_theta.min(), ps_theta.max()],
             interpolation="nearest")
