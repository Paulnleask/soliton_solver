...
class GLRenderer:
    ...
    def bind_sim(self, sim):
        ...
        self._sim = sim
        try:
            self.skyrmion_rotation = float(sim.p_f_h[10])
        except Exception:
            self.skyrmion_rotation = 0.0
    ...
    def _on_key(self, window, key, scancode, action, mods):
        ...
        if key == glfw.KEY_F1:
            ...
            self.set_display_mode(DISPLAY_MAGNETIC_CHARGE)
    ...
    def _on_mouse_button(self, window, button, action, mods):
        ...
        if self.soliton_mode == "skyrmion":
            ...
            if action == glfw.RELEASE:
                ...
                p_f = sim.p_f_h
                ans_bloch = bool(p_f[11] > 0.5)
                ans_neel = bool(p_f[12] > 0.5)
                ans_anti = bool(p_f[13] > 0.5)
...
def _compute_density_for_mode(sim, mode: int):
    ...
    if mode == DISPLAY_ENERGY:
        if hasattr(sim, "compute_energy_density"):
            ...
            return
    if mode == DISPLAY_MAGNETIC_CHARGE:
        if hasattr(sim, "compute_magnetic_charge_density"):
    ...
    try:
        while not renderer.should_close():
            ...
            try:
                ...
                if "magnetic_charge" in obs:
                    state["rho"] = float(obs["magnetic_charge"])
            except Exception:
                pass
            ...
            if renderer.display_mode==DISPLAY_ENERGY:
                use_jet = True
            elif renderer.display_mode == DISPLAY_MAGNETIC_CHARGE:
                use_jet = False
            else:
                use_jet = False
