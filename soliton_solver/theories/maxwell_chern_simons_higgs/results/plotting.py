"""
To run: python -m soliton_solver.theories.maxwell_chern_simons_higgs.results.plotting
"""
import numpy as np
import matplotlib
...
GaugeField0 = load_dat("GaugeField0.dat")

EnergyDensity = load_dat("EnergyDensity.dat")
MagneticFluxDensityXY = load_dat("VortexDensity.dat")
NoetherChargeDensity = load_dat("NoetherChargeDensity.dat")
ElectricChargeDensity = load_dat("ElectricChargeDensity.dat")
...
GaugeField0 = bicubic_upscale(GaugeField0, 2)
...
NoetherChargeDensity = bicubic_upscale(NoetherChargeDensity, 2)
ElectricChargeDensity = bicubic_upscale(ElectricChargeDensity, 2)
...
draw_panel(ax21, xGrid, yGrid, GaugeField0, r"$A_0(\vec{x})$", xLength, yLength, xlim=(xMin,xMax), ylim=(yMin,yMax))

# Noether charge density
ax22 = fig1.add_subplot(gs[1, 1])
draw_panel(ax22, xGrid, yGrid, NoetherChargeDensity, r"$\rho_m(\vec{x})=q^2A_0|\psi|^2$", xLength, yLength, xlim=(xMin,xMax), ylim=(yMin,yMax), cmap="bone")

# Electric charge density
ax23 = fig1.add_subplot(gs[1, 2])
draw_panel(ax23, xGrid, yGrid, ElectricChargeDensity, r"$\rho_e(\vec{x})=-\nabla^2A_0$", xLength, yLength, xlim=(xMin,xMax), ylim=(yMin,yMax), cmap="bone")
