# =========================
# soliton_solver/theories/spin_triplet_superconducting_magnet/kernels.py
# =========================
"""
Core CUDA kernels and device helpers for the superferro model.
...
- Many kernels assume a fixed component layout for Field:
    ...
    Higgs:         a = 6,7,8,9 (psi1r, psi1i, psi2r, psi2i)  (real/imag components)
...
# ---------------- Compute the Higgs 1 norm density ----------------
@cuda.jit(device=True)
def compute_norm_higgs1_point(Field, x, y, p_i):
    """
    Compute |psi_1|^2 at a single lattice site.

    Usage:
    - Device helper used by compute_norm_higgs_kernel().
    - Assumes Higgs components are stored at a=6 (psi1r) and a=7 (psi1i).
    """
...
def compute_norm_higgs1_kernel(en, Field, p_i):
    """
    Compute |psi_1|^2 across the grid.

    Usage:
    - Launch over (xlen, ylen).
    - Writes output into en at component 0:
        en[a=0, x, y] = |psi_1(x,y)|^2
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
...
# ---------------- Compute the Higgs 2 norm density ----------------
@cuda.jit(device=True)
def compute_norm_higgs2_point(Field, x, y, p_i):
    """
    Compute |psi_2|^2 at a single lattice site.

    Usage:
    - Device helper used by compute_norm_higgs_kernel().
    - Assumes Higgs components are stored at a=8 (psi2r) and a=9 (psi2i).
    """
...
def compute_norm_higgs2_kernel(en, Field, p_i):
    """
    Compute |psi_2|^2 across the grid.

    Usage:
    - Launch over (xlen, ylen).
    - Writes output into en at component 0:
        en[a=0, x, y] = |psi_2(x,y)|^2
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
...
def compute_supercurrent_point(Supercurrent, d2fd2x, x, y, p_i):
    ...
    - Zeros the first `number_gauge_fields` components of Supercurrent at (x,y),
...
def do_gradient_step_point(Velocity, Field, EnergyGradient, d1fd1x, d2fd2x, x, y, p_i, p_f):
    ...
    - Computes gradient components g0..g7 corresponding to:
        ...
        6..9: Higgs fields  (psi1r,psi1i,psi2r,psi2i)
    ...
    - p_f: uses p_f[5]=time_step and other model parameters (alpha,beta,gamma,ha,hb1,hb2,hc,q)
    """
    # Parameters
    time_step = p_f[5]
    q = p_f[6]; ha = p_f[7]; hb1 = p_f[8]; hb2 = p_f[9]; hc = p_f[10]
