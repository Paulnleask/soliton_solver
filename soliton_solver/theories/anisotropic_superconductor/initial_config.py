"""
Initial condition kernels for the anisotropic s+id superconductor theory.
...
def create_ground_state_kernel(Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i, p_f):
    ...
    Field[idx_field(4, x, y, p_i)] = u2 * math.cos(theta_diff)
    Field[idx_field(5, x, y, p_i)] = u2 * math.sin(theta_diff)
...
@cuda.jit
def create_initial_configuration_kernel(Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i, p_f):
    """
    ...
    xsize = p_f[0]; ysize = p_f[1]
    u1 = p_f[17]; u2 = p_f[18]
    ainf = p_f[22]; vortex_number = p_f[21]
    vortex1_number = p_f[23]; vortex2_number = p_f[24]
    xcent = (grid[idx_field(0, 0, 0, p_i)] + grid[idx_field(0, xlen-1, ylen-1, p_i)]) / 2.0
    ycent = (grid[idx_field(1, 0, 0, p_i)] + grid[idx_field(1, xlen-1, ylen-1, p_i)]) / 2.0
    r1 = position(xcent, ycent, x, y, grid, p_i)
    rmax = xsize if xsize > ysize else ysize
    th = angle(r1, xcent, ycent, 0.0, x, y, grid, p_i)
    fs = profile_function_superconductor(r1, rmax, 1.0)
    eps = 1e-12
    rr = r1 if r1 > eps else eps
    Field[idx_field(0, x, y, p_i)] = ainf * fs * math.sin(th) / rr
    Field[idx_field(1, x, y, p_i)] = -ainf * fs * math.cos(th) / rr
    Field[idx_field(2, x, y, p_i)] = u1 * fs * math.cos(vortex1_number * th)
    Field[idx_field(3, x, y, p_i)] = u1 * fs * math.sin(vortex1_number * th)
    Field[idx_field(4, x, y, p_i)] = u2 * fs * math.cos(vortex2_number * th)
    Field[idx_field(5, x, y, p_i)] = u2 * fs * math.sin(vortex2_number * th)
    number_total_fields = p_i[4]
    for a in range(number_total_fields):
        Velocity[idx_field(a, x, y, p_i)] = 0.0
        k1[idx_field(a, x, y, p_i)] = 0.0
...
def create_vortex_kernel(Field, grid, pxi, pxj, vortex, p_i, p_f):
    ...
    xlen = p_i[0]; ylen = p_i[1]
    xsize = p_f[0]; ysize = p_f[1]
    vortex_number = p_f[21]; ainf = p_f[22]
    ...
    r1 = position(xcent, ycent, x, y, grid, p_i)
    rmax = xsize if xsize > ysize else ysize
    theta1 = angle(r1, xcent, ycent, 0.0, x, y, grid, p_i)
    fs = profile_function_superconductor(r1, rmax, 2.0)
    phase = (-1.0 if vortex else 1.0) * theta1
    eps = 1e-12
    rr = r1 if r1 > eps else eps
    Field[idx_field(0, x, y, p_i)] -= ainf * fs * math.sin(theta1) / rr
    Field[idx_field(1, x, y, p_i)] += ainf * fs * math.cos(theta1) / rr
    old_r1 = Field[idx_field(2, x, y, p_i)]
    old_i1 = Field[idx_field(3, x, y, p_i)]
    int_r1 = fs * math.cos(vortex_number * phase)
    int_i1 = fs * math.sin(vortex_number * phase)
    new_r1 = (old_r1 * int_r1 - old_i1 * int_i1)
    new_i1 = (old_r1 * int_i1 + old_i1 * int_r1)
    Field[idx_field(2, x, y, p_i)] = new_r1
    Field[idx_field(3, x, y, p_i)] = new_i1
    old_r2 = Field[idx_field(4, x, y, p_i)]
    old_i2 = Field[idx_field(5, x, y, p_i)]
    int_r2 = fs * math.cos(vortex_number * phase)
    int_i2 = fs * math.sin(vortex_number * phase)
    new_r2 = (old_r2 * int_r2 - old_i2 * int_i2)
    new_i2 = (old_r2 * int_i2 + old_i2 * int_r2)
    Field[idx_field(4, x, y, p_i)] = new_r2
    Field[idx_field(5, x, y, p_i)] = new_i2
...
def initialize(*, Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i_d, p_f_d, p_i_h=None, p_f_h=None, grid2d, block2d, config: dict | None = None):
    ...
    create_initial_configuration_kernel[grid2d, block2d](Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i_d, p_f_d)