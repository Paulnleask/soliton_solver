"""
Initialize field configurations for the Ginzburg-Landau superconductor theory.

Examples
--------
Use ``create_ground_state_kernel`` to initialize the uniform ground state.
Use ``create_initial_configuration_kernel`` to initialize a vortex configuration.
"""
import math
...
def angle(r, xcent, ycent, x, y, grid, p_i):
    ...
    Examples
    --------
    Use ``th = angle(r, xcent, ycent, x, y, grid, p_i)`` to build vortex phases.
    """
    theta = 0.0
    if r == 0.0:
        return theta
...
def create_initial_configuration_kernel(Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, vortex_number, p_i, p_f):
    """
    Initialize the vortex configuration and zero the auxiliary buffers.
    ...
    Examples
    --------
    Launch ``create_initial_configuration_kernel[grid2d, block2d](Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, vortex_number, p_i, p_f)`` to initialize the vortex configuration.
    """
    x, y = cuda.grid(2)
    if not in_bounds(x, y, p_i):
        return
...
def initialize(*, Velocity, Field, grid, k1, k2, k3, k4, l1, l2, l3, l4, Temp, p_i_d, p_f_d, p_i_h=None, p_f_h=None, grid2d, block2d, config: dict | None = None):
    """
    Initialize the theory fields using the requested mode.
    ...
    Use ``initialize(..., config={"mode": "initial", "vortex_number": 1.0})`` to initialize the vortex configuration.
    """
    cfg = config or {}
